...
3) ADD, RUN, EXPOSE 등 -> 이후에 이미지 기동해서 컨테이너를 만드는 과정에서 진행되는 프로세스임
 ...
 ADD- 앞쪽에 있는 매개변수를 / 뒤의 매개변수로 복사하는 것
...
ex) EXPOSE 8080 - 8080포트로 전달하게끔 열어놓겠다
...
i(interactive option)현재 쓰는 터미널 상태에서 값을 입력하는 것-> 없으면 값이 입력이 안되
...
![docker서버관련그림](C:\Users\user\Desktop\자연어처리과정\TIL\work\image\docker서버관련그림.png)생성한 2개의 컨테이너가 있다.
